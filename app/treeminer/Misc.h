 ...
 */
Element* test_node(int iter, TreeMinerPattern *eq, int val, int pos) {
	Element *eqn = NULL;
...
*/
void check_ins(vector<TreeMinerProjTrans> *l1, vector<TreeMinerProjTrans> *l2, Element *ins, // inside
		int st1, int st2, int en1, int en2, bool f2, vector<TreeMinerProjTrans>* output_tlist) {
	TreeMinerProjTrans *n1, *n2;
	ival_type cmpval;
...
*/
void check_outs(vector<TreeMinerProjTrans> *l1, vector<TreeMinerProjTrans> *l2, Element *outs,
		int st1, int st2, int en1, int en2, vector<TreeMinerProjTrans>* output_tlist) {
	TreeMinerProjTrans *n1, *n2;
	ival_type cmpval;
...
void get_intersect(vector<TreeMinerProjTrans> *l1, vector<TreeMinerProjTrans> *l2, Element *ins,
		Element *outs, bool f2 = false) { //f2 is true only when we compute F2, set to false for Fk
	TreeMinerProjTrans *n1, *n2; //in Sleuth paper Fig 3 Vertical Format, n1 points to a row of a rectangle
	...
	if((l1->size() + l2->size()) > tauDB_omp){
		//do parallel join
		vector<vector<TreeMinerProjTrans> > ins_tlistOfThread(THREADS);
		vector<vector<TreeMinerProjTrans> > outs_tlistOfThread(THREADS);
		...
		if(ins){
			for(int i = 0; i < THREADS; i++){
				vector<TreeMinerProjTrans>& tlist_i = ins_tlistOfThread[i];
		...
		if(outs) {
			for(int i = 0; i < THREADS; i++){
				vector<TreeMinerProjTrans>& tlist_i = outs_tlistOfThread[i];
