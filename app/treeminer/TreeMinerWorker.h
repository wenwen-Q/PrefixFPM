...
#include "TreeMinerTask.h"

#define ITSZ sizeof(int)
#define buf_sz 2048
#define TRANSOFF 3

class TreeMinerWorker: public Worker<TreeMinerTask> {
private:
	int buf_size;
	int * buf;
	int cur_blk_size;
...
public:
	...
	vector<vector<TreeMinerProjTrans> *> Idlists;

	//function definitions
	TreeMinerWorker(const char *infile, const char *outfolder = "outputs"): Worker(infile, outfolder){
		buf_size = buf_sz;
		buf = new int[buf_sz];
		cur_buf_pos = 0;
		cur_blk_size = 0;
	...
	virtual int getNextTrans(vector<TransT>& transDB) {
		...
		if (binary_input) {
			...
			if (!readall) {
				...
				TransDB().push_back(new TreeMinerTrans(TransAry, TransSz, Tid, Cid));
	...
	void get_valid_trans(TreeMinerTrans &t) {
		int i, j;
		int newTransSz;
	...
	void make_vertical(TreeMinerTrans& t) {
	...
	void alloc_idlists() {
		...
		for (int i = 0; i < NumF1; i++) {
			Idlists[i] = new vector<TreeMinerProjTrans>;// todo check has deleted???
		}
	}
	...
	void get_F2(stack<TreeMinerTask*>& task_queue) {
		int i, j;
		int it1, it2;
		int scnt;
		...
		for (int m = 0; m < TransDB().size(); m++) {
			...
			TreeMinerTrans* validT =  TransDB()[m];
		...
		for (i = 0; i < NumF1; i++) {
			TreeMinerTask* t = new TreeMinerTask;
			t->iter = 2;
			TreeMinerPattern& eq = t->pat;
			for (j = 0; j < NumF1; ++j) {
	...
	void form_f2_lists(TreeMinerPattern& eq) {
		vector<Element *>::iterator ni;
		vector<TreeMinerProjTrans> *l1, *l2;
	...
	virtual void setRoot(stack<TreeMinerTask*>& task_queue){
		get_F1();
		get_F2(task_queue);
	}
