...
class TreeMinerProjTrans: public ProjTrans {
public:
	...
	TreeMinerProjTrans(int t = 0, int l = 0, int u = 0, int d = 0, bool i = false) :
		ProjTrans(t), itscope(l, u), depth(d), induced(i) {
	}

	TreeMinerProjTrans(int t, ival &it, int d, bool i = false): ProjTrans(t) {
		itscope = it;
		depth = d;
		induced = i;
	}

	TreeMinerProjTrans(int t, vector<int> &pscope, int lit, ival &ivl, int d, bool i =
			false): ProjTrans(t){
		itscope = ivl;
		parscope = pscope;
	...
	bool path_equal(TreeMinerProjTrans &idn) {
		int i;
	...
	void project(TreeMinerProjTrans* newProj){
		newProj->parscope = parscope;
		newProj->parscope.push_back(itscope.lb);
	}
};

bool projTrans_cmp(const TreeMinerProjTrans& a, const TreeMinerProjTrans& b){
...
ostream & operator<<(ostream& ostr, TreeMinerProjTrans& idn) {
	ostr << idn.tid << " " << idn.itscope << " -- " << idn.parscope << " , "
			<< idn.depth << " " << idn.induced;
	return ostr;
}

ostream & operator<<(ostream& fout, vector<TreeMinerProjTrans>& idl) {
...
public:
	...
	vector<TreeMinerProjTrans> tlist; //scope list for this item
...
class TreeMinerTrans:public Trans {
public:
	vector<int> TransAry;
	int Tid;

	TreeMinerTrans(int* _TransAry, int _TransSz, int _Tid, int _Cid):Trans(_Cid) {
		for(int i = 0; i< _TransSz; i++)
			TransAry.push_back(_TransAry[i]);
		Tid = _Tid;
	}

	TreeMinerTrans(const TreeMinerTrans& t):Trans(t.tid) {
		TransAry = t.TransAry;
		Tid = t.Tid;
	}
};
