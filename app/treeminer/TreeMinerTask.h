...
#include "../treeminer/Misc.h"
class TreeMinerTask: public Task<TreeMinerPattern, ChildrenT, TreeMinerTrans*> {
public:
	int iter;
	ostream* ostr;
	...
	virtual void setChildren(ChildrenT& children) { // implementing Fig 9 of the Sleuth paper
		...
		for (ni = pat.nlist().begin(); ni != pat.nlist().end(); ++ni) {
			TreeMinerPattern *npat = new TreeMinerPattern;
